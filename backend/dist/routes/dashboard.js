...
router.get('/kpis', async (_req, res) => {
    ...
    const [allOpportunities, closedThisMonth, totalAccounts, recentActivities, staleOpportunities,] = await Promise.all([
        ...
        prisma.activity.findMany({
            where: { createdAt: { gte: (0, date_fns_1.subDays)(now, 7) } },
            include: { user: { select: { id: true, name: true } }, account: { select: { id: true, razonSocial: true } } },
            orderBy: { createdAt: 'desc' },
            take: 10
        }),
        prisma.opportunity.findMany({
            where: { stage: { notIn: ['CERRADO_GANADO', 'CERRADO_PERDIDO'] }, lastStageChange: { lt: (0, date_fns_1.subDays)(now, 14) } },
...
router.get('/team', async (_req, res) => {
    ...
    const stats = await Promise.all(rcs.map(async (rc) => {
        const [activeOps, wonOps, lostOps, activities] = await Promise.all([
            ...
            prisma.activity.count({ where: { userId: rc.id, createdAt: { gte: (0, date_fns_1.subDays)(now, 30) } } })
        ]);
